...
pub fn segment_to_continue(repository: &Repository) -> Result<Option<(String,Option<(String,usize)>)>, RebaseError>
{
    ...
    match lines.next_back() {
        ...
        Some(oid) => {
            let skip_str = lines.next_back().ok_or_else(|| RebaseError::WrongState(repository.state()))?;
            let skip: usize = skip_str.parse().map_err(|_| RebaseError::WrongState(repository.state()))?;
